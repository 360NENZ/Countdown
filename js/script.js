...
// 假设 musicLyric 是预先定义好的LRC格式歌词字符串
...
function parseLyrics(lyricsText) {
  return lyricsText.split('\n').map(function(line) {
    ...
    const [timeStr, text] = line.slice(1).split(']');
    const [min, sec] = timeStr.split(':').map(Number);
    const millisecond = Math.floor((sec % 1) * 1000);
    sec = Math.floor(sec);
    const timestamp = (min * 60 + sec) * 1000 + millisecond;
...
function updateLyricDisplay() {
  const currentTime = Math.floor(musicPlayer.currentTime * 1000);
  if (currentTime !== lastCurrentTime) {
    lastCurrentTime = currentTime;
    let newLineIndex = currentLineIndex;
    
    // 查找当前时间对应的歌词行，如果当前行的下一行时间小于当前时间，则移到下一行
    while (newLineIndex < musicLyricContent.length - 1 && 
           musicLyricContent[newLineIndex + 1].timestamp <= currentTime) {
      newLineIndex++;
    }
    ...
    // 检查是否需要重置歌词到第一行
    if (currentTime >= musicLyricContent[musicLyricContent.length - 1].timestamp) {
      musicPlayer.currentTime = 0; // 重置音乐到开始
      currentLineIndex = 0; // 重置歌词到第一行
      musicLyricDisplay.textContent = musicLyricContent[0].text;
    }
  }
  requestAnimationFrame(updateLyricDisplay); // 递归调用以继续动画帧更新
}

// 监听音乐播放开始事件
musicPlayer.addEventListener('play', function() {
  requestAnimationFrame(updateLyricDisplay); // 开始更新歌词显示
});
