...
function parseLyrics(lyricsText) {
  return lyricsText.split('\n').map(function(line) {
    ...
    if (trimmedLine === '') return null; // 忽略空行
    const [timeStr, text] = trimmedLine.split(']');
    const [mm, ss] = timeStr.split(':').map(Number);
    const timestamp = (mm * 60 + ss) * 1000;
    return { timestamp, text: text.trim() };
  }).filter(item => item !== null); // 过滤掉空行
}
...
function updateLyricDisplay() {
  ...
  // 使用requestAnimationFrame来实现平滑的歌词滚动效果
...
// 初始化歌词显示
musicLyricDisplay.textContent = musicLyricContent[0] ? musicLyricContent[0].text : '歌词载入中...';
